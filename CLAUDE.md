...
> **⚑ Current visual direction: Adaptive Editorial Cinema (ADR 021).** The body of this
> section below describes the **prior Thoughtful Seatmate** direction, now **superseded**
> for the foundation. Current truth: **deep neutral ink** (`#0f1010`/`#171819`/`#222427`/`#2e3135`),
> **paper-white type** (`#f5f2eb`/`#c9c5bc`/`#a5a198`), **neutral inverse primary action**
> (`#f0ece4` on `#0f1010`), a **cinematic coral-red signature** (`#e5636f` / text `#ed7a87` /
> strong `#b83d4f`) reserved for marks/links/active-nav/progress (never actions, selection,
> focus, semantics, or large atmosphere), **Inter only**, **flat-first composition** (depth is
> opt-in), and **restrained rounded-rectangle buttons** (pills only for compact choices, circles
> for icon-only). Current authority: `docs/ui/design-authority-adaptive-editorial-cinema.md`;
> operational rule: `.claude/rules/design-system.md`. ADR 014 (Inter) and ADR 019's
> single-root-theme architecture are preserved; `.theme-thoughtful` is a temporary compatibility
> class name. The warm-graphite/projection-ivory/rose specifics below are historical.

The current design system is a baseline, not a permanent constraint.

The design character is **Adaptive Editorial Cinema**: intelligent editorial cinema — calm enough to choose, clear enough to compare, open enough to explore, personal enough to reflect an evolving taste. "Smart but tired" is one valid posture (the *Decide* mode), not the whole character. *(The "Thoughtful Seatmate" framing below is historical — superseded by ADR 021.)*

Accepted target principles (the **Thoughtful Seatmate visual system is consolidated** — `docs/ui/thoughtful-seatmate-visual-system-closure.md`): a warm graphite / warm-neutral foundation with a **neutral near-black→warm-graphite depth treatment** that is *not* a brand gradient; projection-ivory text; **Inter as the one core sans-serif voice** (everywhere, including long-form Film File prose and exported share artifacts); a **neutral, high-contrast primary action** (projection-ivory fill `#efe7d7` + dark warm text `#221b13`); **ivory-only** confirmation for meaningful selection (projection ivory `#f3ecdf` + redundant non-color cues; no separate warm cue); **one restrained solid rose accent** (`#DD4E83`) used sparingly and never as a primary action, decision signal, semantic state, large atmosphere, or pervasive treatment; the **legacy purple–pink gradient retired from the target** (no memory-only or share-only role, no replacement, no new gradient token); contextual film color kept as accepted appearance research but **deferred** (no accepted extraction method); progressive recommendation depth; brief, purposeful motion. No default visible backups on Tonight; no permanent serif-versus-system split; no plum/purple supporting system; no required numbered case.

The shipped app is **mixed and transitional**: many feature surfaces ship the F4 Newsreader/Inter/rose direction (`/home` also has the numbered I/II case and a neutral primary), while residual global, shared, and legacy areas still contain Outfit, purple/pink styling, and gradient treatments. Treat it as a transitional baseline — do not claim either F3/F4 or the Thoughtful Seatmate direction is uniformly implemented, and do not remove shipped work outside a deliberate, gated re-migration. **Inter** is the accepted single core Latin sans-serif target following the P1 controlled blind comparison (`docs/decisions/014-thoughtful-seatmate-p1-core-voice.md`); Newsreader and Outfit remain part of the transitional baseline until deliberate surface migration. The warm graphite / projection-ivory **foundation token values are now accepted** (`docs/decisions/015-thoughtful-seatmate-p2a-foundation.md`) as fixed prototype/pilot-scoped values — not yet global production tokens. Meaningful **selected/committed confirmation remains ivory-only** (projection ivory `#f3ecdf` supplementary marker + redundant non-color signals; **no separate warm-cue token**; `docs/decisions/016-thoughtful-seatmate-p2b-decision-signal.md`). The contextual-film-color treatment is **accepted in full** — preserve source hue, output lightness **L = 0.62**, retained chroma cap **C = 0.04** (strict envelope, `docs/decisions/017-thoughtful-seatmate-p2c-a-contextual-color-envelope.md`) and aura **compositing alpha 0.14** (`docs/decisions/018-thoughtful-seatmate-p2c-b-aura-strength.md`), single focal-film scope; **no production aura is implemented** and no global contextual-color token exists. The contextual-color **extraction** method remains **unresolved and is not accepted**: edge-context was selected in the synthetic P2C-C comparison but **failed the genuine real-poster holdout (P2C-D)** on transformation stability, alternate-rendition stability, incidental-color rate, product value over showing no aura, and harmful suppression. **Do not implement edge-context as the production extraction method.** No automatic extraction method is accepted and manual deterministic seeds are not a production method (`docs/ui/thoughtful-seatmate-p2c-d-validation-closure.md`). The **long-form Film File reading voice is resolved**: P2D tested a bounded Newsreader long-form exception; it improved sustained reading comfort but **tied Inter-only 90–90**, scored lower on product-voice coherence and mixed-script robustness, and carried greater font-swap reflow, so it is **not accepted** — **Film File long-form body prose remains Inter; do not introduce Newsreader or another serif into it** (`docs/ui/thoughtful-seatmate-p2d-reading-voice-closure.md`). Newsreader stays transitional shipped-baseline usage only (do not remove opportunistically; removal needs a separate scoped migration). The **legacy purple–pink gradient is retired from the target system** — no memory-only or share-only role, no replacement gradient, no new gradient token; existing shipped usage (`--brand-gradient`/`--gradient-primary`, `HP_GRAD`) is transitional migration debt removed only on deliberate surface migration — **do not claim it is already gone from production**. The **P2E gradient-survival study was stopped before a valid blind review and produced no decision** (no winner, no decision rule applied); gradient survival is resolved by simplification — **retired** (closure: `docs/ui/thoughtful-seatmate-visual-system-closure.md`).

Current design authority: **`docs/ui/design-authority-adaptive-editorial-cinema.md`** ([ADR 021](docs/decisions/021-adaptive-editorial-cinema-foundation.md)). The prior `docs/ui/design-authority-thoughtful-seatmate.md` and `docs/ui/thoughtful-seatmate-visual-system-closure.md` are **historical (superseded by ADR 021)** — they record the prior direction, not the current target. `docs/ui/design-authority-f3.md` is also historical.

...
| [`csp-report-only-verification-f9g1.md`](csp-report-only-verification-f9g1.md) | **F9G.1 CSP report-only production verification** — merged F9G (PR #174, squash `86a0ebab`); confirmed `content-security-policy-report-only` is **live on prod** (no enforcing CSP; other headers intact); app renders + fonts/TMDB/Supabase/Sentry all work; the **only** violation is the Cloudflare-injected inline RUM beacon (`script-src-elem`, landed in Sentry → Security as `FEELFLICK-APP-5`); reporting pipeline verified end-to-end; enforcement-readiness decision + cleanup steps. Verification only — no code change. |

...
test.describe('Film File — authenticated, intercepted', () => {
  test('A — decision dossier happy path', async ({ page }) => {
    ...
    // Decision-zone order: Synopsis → Providers → Your Take(compact).
    ...
    // §15 spoiler boundary is shown; no post-watch portrait or social proof pre-watch.
    await expect(page.getByText(/The page stops before interpretation/i)).toBeVisible()
    ...
    // Pre-watch tail (ExplorationTail always visible, DetailsSection always visible).
    await expect(page.getByText('Continue the thread')).toBeVisible()
  ...
  test('B — exploration cap + social gating (social is watched-gated)', async ({ page }) => {
    const ledger = await installMovieFixture(page)
    const errors = attachGuards(page)
    await gotoFilmFile(page)
    ...
    // ExplorationTail is always visible (no disclosure toggle required).
    const explore = page.locator('.ff-movie-exploration-tail')
  ...
  test('E — Mark Watched success then failure', async ({ page }) => {
    ...
    // The post-watch chapter mounts on settled success.
  ...
  test('F — Your Take rating persistence (start watched)', async ({ page }) => {
    ...
    await expect(page.getByLabel('Your star rating')).toBeVisible({ timeout: 10_000 })
    await expect(page.getByText(/Mark this film watched above to rate it/i)).toHaveCount(0)
    // Non-Parasite films show no portrait placeholder (GenericPostWatchState removed).

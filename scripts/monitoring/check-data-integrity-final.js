...
async function checkDataIntegrity() {
  ...
  // FIX: Get actual data with correct columns
  ...
  const missingRecommendationScore = allMovies?.filter(m => m.recommendation_score === null).length || 0;
  
  if (missingQualityScore > 0) {
    console.log(`\n  ℹ️  Movies missing quality_score: ${missingQualityScore}`);
  }
  if (missingRecommendationScore > 0) {
    console.log(`  ℹ️  Movies missing recommendation_score: ${missingRecommendationScore}`);
  }
  ...
  const noCast = allMovies?.filter(m => m.cast_count === 0 || m.cast_count === null).length || 0;
  ...
  console.log(`  ℹ️  Movies with 0 cast: ${noCast} (${(100 - castCoverage).toFixed(1)}%)`);
  ...
  if (moviesWithoutKeywords > 0) {
    console.log(`  ℹ️  Movies with no keywords: ${moviesWithoutKeywords}`);
  }
  ...
  const { data: allRatings } = await supabase
    ...
    .select('movie_id, imdb_rating, rotten_tomatoes_rating, metacritic_rating');
  ...
  const withRT = allRatings?.filter(r => r.rotten_tomatoes_rating !== null).length || 0;
  const withMetacritic = allRatings?.filter(r => r.metacritic_rating !== null).length || 0;
  ...
  console.log(`  Total Movies: ${totalMovies.toLocaleString()}`);
  console.log(`  Complete Status: ${statusCount['complete'] || 0} (${((statusCount['complete'] / totalMovies) * 100).toFixed(1)}%)`);

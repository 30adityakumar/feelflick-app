...
export default function HeroTopPick() {
  ...
  // Live watch providers from TMDB (JustWatch under the hood)
  ...
  useEffect(() => {
    ...
    async function loadProviders() {
      try {
        ...
        if (!res.ok) {
          console.error('[HeroTopPick] Failed to fetch watch providers', res.status)
        ...
        console.error('[HeroTopPick] Error fetching watch providers', err)
        if (!cancelled) setProviders(null)
      } finally {
  ...
  }, [movie?.id, movie?.tmdb_id, TMDB_API_KEY])
  ...
  // Genres from DB: ["Drama","History","War"]
      ...
      <div className="absolute inset-0">
        ...
        <div className="flex flex-col sm:flex-row gap-5 sm:gap-6 lg:gap-10 w-full">
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-end">
            {/* Label */}
            <div
              className={`flex items-center gap-2 mb-3 sm:mb-4 transition-all duration-500 ${
                revealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
            >
            ...
            >
              ...
              {genreNames.slice(0, 2).map((name, i) => (
                <span
                  key={`${name}-${i}`}

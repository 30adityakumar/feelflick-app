...
export default function ToastNotification({ message, subtext, ctaLabel, ctaHref, onDismiss, duration = 8000 }) {
  ...
  return (
    ...
    >
      <div className="relative flex items-start gap-3 rounded-2xl bg-[#1a1525] border border-[#DD4E83]/30 px-5 py-4 shadow-xl shadow-black/50">
        <Sparkles className="h-5 w-5 text-[#DD4E83] shrink-0 mt-0.5" />
        <div className="flex-1 min-w-0">
          ...
          {ctaLabel && ctaHref && (
            <Link
              to={ctaHref}
              onClick={onDismiss}
              className="text-xs text-[#DD4E83]/80 hover:text-[#DD4E83] font-medium mt-2 block transition-colors"
            >
              {ctaLabel}
            </Link>
          )}

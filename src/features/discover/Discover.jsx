...
function AudioToggle() {
  ...
  return (
    <button onClick={() => { const n = !muted; FFAudio.setMuted(n); setMuted(n); }} title={muted?'Sound off':'Sound on'} aria-label={muted?'Sound off':'Sound on'} style={{ position:'fixed', bottom:24, left:24, width:44, height:44, borderRadius:999, background:'var(--color-surface-raised, rgba(18,11,28,0.85))', backdropFilter:'blur(12px)', border:`1px solid var(--color-border-subtle, rgba(255,255,255,0.14))`, color:muted?'var(--color-text-muted, rgba(255,255,255,0.45))':'var(--color-text-primary, #A78BFA)', cursor:'pointer', zIndex:60, display:'inline-flex', alignItems:'center', justifyContent:'center' }}>

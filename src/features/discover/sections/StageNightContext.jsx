  ...
  return (
    <section className="ff-night-context ff-discover-section" style={{ position:'relative', minHeight:'82vh', animation:'ff-fade 0.5s ease' }}>
      <div style={{ textAlign:'center', marginBottom:28 }}>
        <div style={{ fontSize:10, fontWeight:700, letterSpacing:'0.32em', textTransform:'uppercase', color:HP.purple, marginBottom:14 }}>Tonight · 2 of 2</div>
        <h1 style={{ fontFamily:'Outfit', fontSize:'clamp(28px, 5vw, 56px)', lineHeight:1.05, fontWeight:300, letterSpacing:'-0.04em', color:HP.text, margin:0 }}>
          A few details, <em style={{ fontStyle:'italic', fontWeight:400, color:blendHex }}>already filled in.</em>
        </h1>
        <p style={{ marginTop:14, fontFamily:'Outfit, Inter, sans-serif', fontSize:14, color:HP.textMuted, fontStyle:'italic' }}>Keep this starting point, or adjust anything that feels off.</p>
      </div>

      <div style={{ maxWidth:760, margin:'0 auto' }}>
        {/* Summary — quiet 2×2 grid; value labels read from the option tables. */}
        <div style={{ fontSize:9, fontWeight:700, letterSpacing:'0.22em', textTransform:'uppercase', color:HP.textMuted, fontFamily:'Outfit', marginBottom:10 }}>Tonight’s starting point</div>
        ...
        <button
          ...
          style={{ marginTop:16, padding:'10px 18px', borderRadius:999, background:'transparent', border:`1px solid ${HP.border}`, color:HP.textSoft, fontFamily:'Outfit', fontSize:12, fontWeight:600, letterSpacing:'0.02em', cursor:'pointer' }}
        >
          ...
          <div id="ff-night-details" className="ff-night-details" style={{ marginTop:18, display:'flex', flexDirection:'column', gap:24, animation:'ff-fade 0.3s ease' }}>
            {GROUPS.map(g => (
              <fieldset key={g.id} className="ff-night-fieldset" style={{ border:'none', padding:0, margin:0, minWidth:0 }}>
                <legend style={{ fontSize:10, fontWeight:700, letterSpacing:'0.22em', textTransform:'uppercase', color:HP.purple, fontFamily:'Outfit', padding:0, marginBottom:12 }}>{g.legend}</legend>
      ...
      <div className="ff-stage-action-bar">
        ...
        <div className="ff-stage-action-bar__meta" style={{ fontSize:11, color:HP.textFaint, fontFamily:'Outfit' }}>These details shape tonight’s ranking.</div>
        <button onClick={()=>{ playContinueCue(); onNext(); }} style={{ padding:'14px 28px', borderRadius:999, background:HP_GRAD, border:'none', color:'#fff', fontFamily:'Outfit', fontSize:13, fontWeight:600, letterSpacing:'0.04em', cursor:'pointer', boxShadow:'0 12px 30px -10px rgba(236,72,153,0.5)' }}>Find my film →</button>
      </div>
    </section>
  )
}

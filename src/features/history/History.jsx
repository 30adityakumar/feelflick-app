  ...
  return (
    <section className="ff-hist-section ff-hist-section--masthead" style={{ padding:'40px 88px 12px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:18, flexWrap:'wrap' }}>
        <Eyebrow spacing="0.32em" size={10} color={ROSE}>Your library</Eyebrow>
      ...
      <h1 className="ff-hist-hero" style={{ fontFamily:'var(--font-editorial)', fontSize:72, lineHeight:0.96, fontWeight:400, letterSpacing:'-0.03em', color:HP.text, margin:0, textWrap:'balance' }}>
        Your <em style={{ fontStyle:'italic', fontWeight:400, color:HP.textSoft }}>diary.</em>
      </h1>
      <p style={{ marginTop:16, fontFamily:'Inter, sans-serif', fontSize:16, color:HP.textSoft, maxWidth:620, lineHeight:1.55 }}>
      ...
      <div className="ff-hist-pulse-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:20 }}>
        {items.map(s => (
          <div key={s.label} style={{ padding:'20px 22px', borderRadius:6, background:'rgba(255,255,255,0.025)', border:`1px solid ${HP.border}` }}>
            ...
            <div style={{ fontFamily:'var(--font-editorial)', fontSize:44, fontWeight:400, color:HP.text, letterSpacing:'-0.03em', lineHeight:1 }}>{s.value}</div>
            <div style={{ marginTop:6, fontSize:11, color:HP.textSoft, fontFamily:'Inter, sans-serif', fontStyle:'italic' }}>{s.hint}</div>
          </div>
        ))}
      </div>
    </section>
    ...
    <section className="ff-hist-section ff-hist-filterbar" style={{ padding:'40px 88px 20px', borderTop:`1px solid ${HP.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', gap:24, flexWrap:'wrap' }}>
      ...
      <div className="ff-hist-filterbar-controls" style={{ display:'flex', alignItems:'center', gap:14, flexWrap:'wrap' }}>
        <input
          ...
          style={{ padding:'9px 14px', borderRadius:999, background:'rgba(255,255,255,0.04)', border:`1px solid ${HP.border}`, color:HP.text, fontFamily:'Inter, sans-serif', fontSize:12, outline:'none', minWidth:240 }}
        />
    ...
    <section className="ff-hist-section ff-hist-group" style={{ padding:'56px 88px', borderTop:`1px solid ${HP.border}` }}>
      <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:24, gap:16, flexWrap:'wrap' }}>
        <div style={{ display:'flex', alignItems:'baseline', gap:18 }}>
          <h2 style={{ fontFamily:'var(--font-editorial)', fontSize:48, fontWeight:400, color:HP.text, letterSpacing:'-0.025em', margin:0 }}>{month.split(' ')[0]}</h2>
          <span style={{ fontFamily:'Inter, sans-serif', fontSize:20, color:HP.textMuted, letterSpacing:'-0.01em' }}>{month.split(' ')[1]}</span>
        </div>
        <div style={{ fontSize:11, color:HP.textMuted, fontFamily:'Inter, sans-serif', letterSpacing:'0.08em', textTransform:'uppercase' }}>{entries.length} {entries.length===1?'film':'films'}</div>
      </div>
          ...
          return (
            <div key={day}>
              <div className="ff-hist-day-header" style={{
                ...
                <span style={{ fontFamily:'Inter, sans-serif', fontSize:18, fontWeight:500, color:HP.text, letterSpacing:'-0.01em' }}>
                  {dayDate}
                </span>
                {weekday && <span style={{ fontFamily:'Inter, sans-serif', fontSize:13, color:HP.textMuted, letterSpacing:'0.01em' }}>· {weekday}</span>}
                <span style={{ fontSize:10, color:HP.textFaint, fontFamily:'Inter, sans-serif', letterSpacing:'0.10em', textTransform:'uppercase' }}>
                  ...
                  <div key={e.id} role="listitem" className="ff-hist-row" style={{ display:'grid', gridTemplateColumns:'64px 1fr auto auto', gap:24, alignItems:'flex-start', padding:'20px 0', borderBottom:`1px solid ${HP.border}` }}>
                    ...
                    <div style={{ minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                        <h3 className="ff-hist-row__title" style={{ fontFamily:'Inter, sans-serif', fontSize:20, fontWeight:500, color:HP.text, letterSpacing:'-0.015em', margin:0, overflowWrap:'anywhere' }}>{e.title}</h3>
                        {e.year && <span style={{ fontSize:11, color:HP.textMuted, fontFamily:'Inter, sans-serif', letterSpacing:'0.04em' }}>{e.year}{e.dir && e.dir !== '—' ? ` · ${e.dir}` : ''}</span>}
                      </div>
                      ...
                      {e.review && (
                        <div style={{ marginTop:14 }}>
                          ...
                          <div style={{ fontSize:9, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase', color:HP.textFaint, fontFamily:'Inter, sans-serif', marginBottom:6 }}>Your review</div>
                          <p style={{ margin:0, fontSize:14, lineHeight:1.55, color:HP.textSoft, fontFamily:'var(--font-editorial)', fontStyle:'italic', borderLeft:`2px solid ${e.moodHex}55`, paddingLeft:14, textWrap:'pretty' }}>
                            &ldquo;{e.review}&rdquo;
                          </p>
                        </div>
                      )}
                    </div>
                    <span className="ff-hist-row__runtime" style={{ fontSize:11, color:HP.textFaint, fontFamily:'Inter, sans-serif', letterSpacing:'0.06em', textTransform:'uppercase', paddingTop:8 }}>{e.runtime ? `${e.runtime}m` : ''}</span>
                    <button
                      type="button"
...
function EmptyState() {
  ...
  return (
    <section className="ff-hist-section" style={{ padding:'72px 88px 96px', textAlign:'center', borderTop:`1px solid ${HP.border}` }}>
      ...
      <h2 style={{ fontFamily:'Inter, sans-serif', fontSize:36, lineHeight:1, fontWeight:500, letterSpacing:'-0.025em', color:HP.text, margin:'0 0 14px 0' }}>Mark something watched.</h2>
      <p style={{ margin:'0 auto 28px', maxWidth:480, fontSize:14, color:HP.textMuted, fontFamily:'Inter, sans-serif', lineHeight:1.6 }}>
        Hit Mark Watched on any film detail page and it lands here, grouped by month with your rating and a note line.
      </p>
      <button
        type="button"
        onClick={() => navigate('/home')}
        style={{ padding:'12px 22px', borderRadius:999, background:'#DD4E83', border:'none', color:'#fff', fontFamily:'Inter, sans-serif', fontSize:13, fontWeight:600, cursor:'pointer' }}
...
function PageSkeleton() {
  ...
  return (
    <div role="status" aria-busy="true" style={{ padding:'80px 88px' }}>
      ...
      <div aria-hidden="true" className="animate-pulse" style={{ background:'rgba(255,255,255,0.08)', height:80, width:'40%', borderRadius:8, marginBottom:48 }} />
  ...
  return (
    <div style={{ minHeight:'60vh', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div role="alert" style={{ textAlign:'center', maxWidth:520 }}>
        ...
        <h1 style={{ fontFamily:'Inter, sans-serif', fontSize:36, fontWeight:500, color:HP.text, margin:'0 0 14px 0', letterSpacing:'-0.02em' }}>We couldn&rsquo;t load your Diary.</h1>
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          <button type="button" onClick={onRetry} className="ff-hist-error-btn" style={{ minHeight:44, padding:'12px 22px', borderRadius:999, background:'#DD4E83', color:'#fff', border:'none', cursor:'pointer', fontFamily:'Inter, sans-serif', fontSize:14, fontWeight:600 }}>Try again</button>
          <button type="button" onClick={onHome} className="ff-hist-error-btn" style={{ minHeight:44, padding:'12px 22px', borderRadius:999, background:'transparent', color:'rgba(250,250,250,0.85)', border:`1px solid ${HP.border}`, cursor:'pointer', fontFamily:'Inter, sans-serif', fontSize:14, fontWeight:600 }}>Go to Home</button>
        </div>
      </div>
    </div>
  );
      ...
      <div data-library-fallback tabIndex={-1} aria-label="Your diary entries" className="ff-hist-collection" style={{ outline:'none' }}>
        {grouped.length === 0 && (
          ...
          <section className="ff-hist-section" style={{ padding:'72px 88px', textAlign:'center', borderTop:`1px solid ${HP.border}` }} role="status">
            <div style={{ fontFamily:'Inter, sans-serif', fontSize:24, color:HP.textMuted, fontStyle:'italic' }}>
              {query.trim()

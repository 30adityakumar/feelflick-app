...
import { HP, MOOD_META, SLOT_LABELS } from './data'
...
export function BriefMasthead({ user }) {
  ...
  const { issueNum, volume } = currentEdition()
  return (
    <section style={{ padding: '72px 88px 40px', display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'flex-end', gap: 56, borderBottom: `1px solid ${HP.border}` }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 26 }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.18em', textTransform: 'uppercase', color: HP.textMuted, fontFamily: 'Outfit' }}>Issue {issueNum} · Vol. {volume} · For {user.name}</div>
        </div>
        <h1 style={{ fontFamily: 'Outfit, Inter, sans-serif', fontSize: 104, lineHeight: 0.92, fontWeight: 600, letterSpacing: '-0.05em', color: HP.text, margin: 0, textWrap: 'balance' }}>
          Tonight&rsquo;s <em style={{ fontStyle: 'italic', fontWeight: 400, color: HP.textSoft }}>edit</em>.
        </h1>
        <div style={{ marginTop: 28, fontSize: 15, lineHeight: 1.65, color: HP.textSoft, maxWidth: 580, fontFamily: 'Outfit, Inter, sans-serif' }}>
          Three films, hand-picked from your taste signal. <em>Not a feed</em> &mdash; a short list. Pick one and the others rest until tomorrow.
        </div>
        <div style={{ marginTop: 32, display: 'flex', alignItems: 'center', gap: 24, fontSize: 11, color: HP.textMuted, fontFamily: 'Outfit', letterSpacing: '0.04em' }}>
          ...
          <span style={{ width: 1, height: 12, background: HP.border }} />
          <span>{user.watched} film{user.watched === 1 ? '' : 's'} logged</span>
        </div>
      </div>
...
function BriefingCard({ film, idx, label, rationale, featured, onWatch, onSave, onSkip, saved }) {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <h3 style={{ fontFamily: 'Outfit, Inter, sans-serif', fontSize: featured ? 44 : 32, lineHeight: 0.98, fontWeight: 600, letterSpacing: '-0.03em', color: HP.text, margin: '0 0 14px 0', textWrap: 'balance' }}>{film.title}</h3>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, color: HP.textMuted, marginBottom: 18, fontFamily: 'Outfit', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
          <span>{film.year}</span><span style={{ width: 3, height: 3, borderRadius: 999, background: HP.textFaint }} />
          <span>{film.runtime} min</span><span style={{ width: 3, height: 3, borderRadius: 999, background: HP.textFaint }} />
          <span>{film.director}</span>
        </div>
        <p style={{ fontSize: 15, lineHeight: 1.6, color: HP.textSoft, margin: '0 0 22px 0', fontFamily: 'Outfit, Inter, sans-serif', fontStyle: 'italic', textWrap: 'pretty' }}>&ldquo;{rationale}&rdquo;</p>
        <div style={{ display: 'flex', gap: 0, paddingTop: 18, borderTop: `1px solid ${HP.border}` }}>
          <button type="button" onClick={() => onWatch?.(film)} style={{ fontSize: 12, fontWeight: 600, color: HP.text, background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 16px 4px 0', fontFamily: 'Outfit', letterSpacing: '0.04em', display: 'inline-flex', alignItems: 'center', gap: 8, borderRight: `1px solid ${HP.border}` }}>
            Watch <span style={{ fontSize: 14, lineHeight: 1, transform: hover ? 'translateX(2px)' : 'none', transition: 'transform 0.3s ease' }}>&rarr;</span>
          </button>
          <button type="button" onClick={() => onSave?.(film)} disabled={saved} style={{ fontSize: 12, fontWeight: 500, color: saved ? HP.purple : HP.textMuted, background: 'transparent', border: 'none', cursor: saved ? 'default' : 'pointer', padding: '4px 16px', fontFamily: 'Outfit', borderRight: `1px solid ${HP.border}` }}>{saved ? 'Saved ✓' : 'Save'}</button>
          <button type="button" onClick={() => onSkip?.(film)} style={{ fontSize: 12, fontWeight: 500, color: HP.textMuted, background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 16px', fontFamily: 'Outfit' }}>Skip</button>
        </div>
      </div>
    </article>
  )
...
export function TheBriefing({ currentMood, shuffleSeed = 0, onWatch, onSave, onSkip, onReshuffle, savedIds }) {
  const { moods } = useHomeData()
  const moodEntry = moods.find(m => m.id === currentMood.id)
  const picks = useMemo(() => {
    if (!moodEntry) return []
    return shuffleBySeed(moodEntry.films || [], shuffleSeed).slice(0, 3)
  }, [moodEntry, shuffleSeed])
  const labels = SLOT_LABELS[currentMood.id] || ["Tonight's pick", 'Mood match', 'From your DNA']
  const rationaleMap = moodEntry?.rationale || {}

  return (
    <section style={{ padding: '64px 88px 80px' }}>
      {picks.length === 0 ? (
        <div style={{ padding: '40px 0', textAlign: 'center', fontFamily: 'Outfit, Inter, sans-serif', color: HP.textMuted, fontSize: 14, fontStyle: 'italic' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr 1fr', gap: 56 }}>
          {picks.map((film, i) => (
            <BriefingCard
              key={`${currentMood.id}-${film.key}-${shuffleSeed}`}
              film={film} idx={i} label={labels[i]} rationale={rationaleMap[film.key]} featured={i === 0}

...
export default function LandingHero() {
  return (
    <section
      ...
      // Sit just under the fixed TopNav (TopNav sets --topnav-h)
      style={{ marginTop: 'var(--topnav-h, 72px)' }}
    >
      {/* Optional collage layer if you use it */}
      <div className="feelflick-landing-bg" aria-hidden />

      {/* ====== BACKGROUND STACK (logo-inspired) ====== */}
      ...
      {/* Faint bluish “ribbon” accent (logo-inspired) */}
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-[44%] -z-0 h-[28vmax] w-[130vmax] -translate-x-1/2 -rotate-6 opacity-50"
        style={{
          background:
            'linear-gradient(90deg, rgba(68,171,255,.09), rgba(0,204,255,.14) 42%, rgba(68,171,255,.10) 78%)',
          filter: 'blur(18px)',
          WebkitMaskImage:
            'radial-gradient(120% 55% at 50% 50%, #000 42%, transparent 76%)',
          maskImage:
            'radial-gradient(120% 55% at 50% 50%, #000 42%, transparent 76%)',
        }}
      />

      {/* ====== CONTENT: fills exactly screen minus header & footer ====== */}
      <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6">
        <div
          className="grid place-items-center py-8 sm:py-10"
          style={{
            // 100svh minus fixed TopNav and fixed Footer heights
            height:
        ...
        >
          <div className="mx-auto w-full max-w-3xl text-center md:max-w-2xl">
            ...
            {/* Single CTA (slightly elongated) */}
            <div className="mx-auto mt-7 flex max-w-sm justify-center">
              <Link
                to="/auth/sign-up"

...
export default function LandingHero() {
  // TMDb key: v3 string or v4 bearer
  ...
  useEffect(() => {
    ...
    rotRef.current = setInterval(() => setI((x) => (x + 1) % items.length), 6000)
  ...
  const prev = items[(i - 1 + (items.length || 1)) % (items.length || 1)]
  const prevPath = prev?.backdrop_path || prev?.poster_path

  return (
    <section
      ref={heroRef}
      // KEY: make hero start after the fixed TopNav; uses measured CSS var with a 72px fallback
    ...
    >
      ...
      {/* Posters as full-bleed background (subtle) */}
      ...
      {/* Keep your bluish/radial overlay exactly */}
      <div
        aria-hidden="true"
      ...
      {/* Content: perfectly centered vertically within the remaining viewport */}
      <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6">
        <div
          className="grid items-center py-10 sm:py-12"
          style={{ minHeight: 'calc(100svh - var(--topnav-h, 72px))' }}
        >

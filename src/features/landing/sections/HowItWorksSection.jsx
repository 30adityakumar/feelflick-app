...
function StepCard({ step, index, isVisible, onHover, isHovered }) {
  const { number, icon, title, description, badge, color } = step
  ...
  return (
    ...
    >
      ...
      {/* Badge */}
      <div
        className={`inline-flex items-center px-3.5 py-2 rounded-lg border ${colors.badge} backdrop-blur-sm shadow-md transition-all duration-300 ${isHovered ? 'scale-105' : 'scale-100'}`}
      >

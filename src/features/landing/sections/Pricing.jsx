...
export default function Pricing(){
  ...
  return(
    <SectionShell id="pricing" tone="panel" padding="160px 32px" innerStyle={{maxWidth:880,textAlign:'center'}}>
        <Reveal>
          <Eyebrow color={ROSE} style={{marginBottom:24}}>Pricing</Eyebrow>
        ...
        <Reveal delay={150}>
          ...
          <div style={{maxWidth:480,margin:'56px auto 0',padding:'48px 44px',borderRadius:16,background:`linear-gradient(160deg,${ROSE}15,transparent 80%)`,border:`1px solid ${ROSE}55`,position:'relative',overflow:'hidden'}}>
            <div aria-hidden style={{position:'absolute',top:-40,right:-40,width:200,height:200,borderRadius:999,background:`radial-gradient(circle,${ROSE}30,transparent 70%)`,filter:'blur(30px)'}}/>
            <div style={{position:'relative'}}>
              ...
              <div style={{display:'flex',alignItems:'baseline',justifyContent:'center',gap:8,marginBottom:32}}>
                <span style={{fontFamily:'Inter, sans-serif',fontSize:84,fontWeight:200,color:C.text,letterSpacing:'-0.055em',lineHeight:0.9}}>$0</span>
              ...
              <ul style={{margin:0,padding:0,listStyle:'none',display:'flex',flexDirection:'column',gap:14,textAlign:'left',maxWidth:380,marginLeft:'auto',marginRight:'auto'}}>
                {['Unlimited picks, any hour','Cinematic DNA, forever','The Briefing if you want it','No ads. No upsells. No algorithm tax.'].map(t=>
                  <li key={t} style={{display:'flex',alignItems:'center',gap:12,fontFamily:'Inter',fontSize:16,color:C.textMid}}>
                    <span style={{width:18,height:18,borderRadius:999,background:`${ROSE}20`,color:ROSE,display:'inline-flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>

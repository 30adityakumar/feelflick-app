      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: primaryLead ? 12 : 0 }}>
            ...
            {fitBand && (
              <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: 7 }}>
                ...
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10.5, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: HP.textMuted }}>
        ...
        {chips.length > 0 && (
          <div style={{ display: 'flex', gap: 8, marginTop: 18, flexWrap: 'wrap' }}>
            {chips.map(c => (
              <span key={c} style={{ padding: '4px 10px', borderRadius: RADIUS.pill, background: 'rgba(255,255,255,0.05)', border: `1px solid ${HP.border}`, fontFamily: 'Inter, sans-serif', fontSize: 11, fontWeight: 600, letterSpacing: '0.04em', color: HP.textSoft }}>{c}</span>
            ))}
          </div>
        )}

        {showNudge && (
          <p style={{ margin: '16px 0 0 0', fontSize: 12.5, lineHeight: 1.5, color: HP.textMuted, fontFamily: 'Inter, sans-serif' }}>

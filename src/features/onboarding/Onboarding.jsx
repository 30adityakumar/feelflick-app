...
export default function Onboarding() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Try FeelFlick data: most watched last 90 days (top 6)
      ...
      // TMDb fallback: popular now (top 6)
      try {
        const r = await fetch(
  ...
  async function saveAndGo(opts = {}) {
    ...
    try {
      ...
      // Ensure profile exists (fixes FK error when writing movies_watched)
      await ensureUserRowOrFail(session.user);

      // Update latest email/name (non-blocking)
      ...
      // Watched picks (store in movies_watched with conflict on user_id+movie_id)
      if (!skipMovies && watchlist.length) {
        const uniq = Array.from(new Map(watchlist.map((m) => [m.id, m])).values());
        const rows = uniq.map((m) => ({
          user_id,
          movie_id: m.id, // Note: may be TMDb id if coming from search/fallback
          title: m.title ?? null,
        ...
        // Upsert via RPC (recommended) or fallback to single-row loop if needed
        // Here: batched upsert respecting unique (user_id, movie_id)
  ...
  return (
    ...
    >
      ...
      <div className="relative isolate w-[min(92vw,920px)] rounded-[22px] p-[1px] bg-[linear-gradient(135deg,rgba(254,146,69,.45),rgba(235,66,59,.35),rgba(45,119,255,.35),rgba(0,209,255,.35))] shadow-[0_40px_120px_rgba(0,0,0,.55)] -translate-y-[3vh] md:-translate-y-[4vh]">
        {/* soft brand glow */}
        <div
          aria-hidden
        ...
        <div
          ...
          style={{
            height:
              "min(640px, calc(100svh - var(--topnav-h,72px) - var(--footer-h,0px) - 44px))",
          }}
        >
          {/* header */}

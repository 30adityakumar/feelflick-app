...
export default function Onboarding() {
    ...
    (async () => {
      try {
        ...
        if (!wErr && watched && watched.length >= 6) {
          ...
          for (const r of watched) counts.set(r.movie_id, (counts.get(r.movie_id) || 0) + 1);
  ...
  // Page itself should size to the *visible* viewport and respect the safe area.
  return (
    <div
      className="px-3 md:px-0"
      style={{
        height: "100dvh",                 // ← excludes iOS Safari toolbars
        minHeight: "100dvh",
        paddingTop: 0,                    // header hidden during onboarding
        paddingBottom: "max(env(safe-area-inset-bottom,0px),12px)", // ← keep CTAs above home bar
        display: "grid",
        placeItems: "center",
        overflowX: "hidden",
      }}
    >
      {/* Card wrapper — no upward shift on small screens; keep on md+ */}
      <div
        className="
          relative isolate w-[min(92vw,920px)] rounded-[22px] p-[1px]
          ...
          translate-y-0 md:-translate-y-[4vh]
          mb-[max(env(safe-area-inset-bottom,0px),12px)]   /* extra breathing room above toolbar */
        "
      >
        {/* soft brand glow */}
        <div
        ...
        <div
          ...
          style={{
            height: "min(640px, calc(100dvh - 24px))", // use d*vh here as well
          }}
        >
          {/* header */}
          ...
          {/* footer actions — give extra safe-area padding */}
          <div
            className="shrink-0 px-5 sm:px-6 pb-5 sm:pb-6"
          ...
          >
            {step === 1 ? (
              ...
              <div className="flex items-center justify-center gap-5">
                <button className="text-[13px] font-semibold text-white/80" onClick={() => setStep(1)} disabled={loading}>
                  &lt; Back
                </button>
                <button
                  className="px-6 py-2.5 rounded-2xl font-extrabold text-[15px] text-white"

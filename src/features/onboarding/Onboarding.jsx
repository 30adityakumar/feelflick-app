...
export default function Onboarding() {
    ...
    return (
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-gradient-to-br from-[#667eea] via-[#764ba2] to-[#f093fb]">
        <div className="text-center px-6 animate-fade-in">
          <Star className="h-20 w-20 text-white mx-auto fill-current animate-bounce-gentle" />
  ...
  return (
    <div className="fixed inset-0 bg-[#0B1120] flex flex-col min-h-screen">
      ...
      <div className="relative z-10 flex flex-col h-full max-w-6xl mx-auto w-full">
        {step >= 0 && <ProgressIndicator step={step} />}
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background:rgba(102,126,234,0.3);border-radius:5px;}
...
function ProgressIndicator({ step }) {
  const labels = ['Select Genres', 'Add Movies']
  return (
    <div className="flex-none px-6 py-5 border-b border-white/5 bg-[#0B1120]">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-white/50 uppercase tracking-wider">
          Step {step + 1} of 2
        </span>
        <span className="text-xs font-semibold text-white/70">{labels[step]}</span>
      </div>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-center px-6 py-12 overflow-y-auto custom-scrollbar">
      <div className="max-w-xl mx-auto space-y-8 animate-fade-in">
        <Sparkles className="h-14 w-14 text-[#667eea] mx-auto mb-4" />
...
function StepGenres({ GENRES, selectedGenres, toggleGenre, error, loading, onNext, onBack }) {
  ...
  return (
    <div className="h-full flex flex-col px-6 py-3 pb-[calc(70px+env(safe-area-inset-bottom))]">
      <div className="flex-none text-center mb-3">
        <h2 className="text-3xl sm:text-4xl font-black text-white mb-2">
          What do you love to watch?
        </h2>
        <p className={`text-sm font-medium ${feedback.color} transition-colors duration-300`}>{feedback.text}</p>
      </div>
      {error && (
        <div className="flex-none max-w-md mx-auto mb-2 p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-sm text-center backdrop-blur-sm">
          {error}
        </div>
      )}
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar -mx-2 px-2" style={{paddingBottom:'64px'}}>
          ...
          {GENRES.map((g, idx) => {
            ...
            return (
              <button
                ...
                className={`relative h-16 rounded-2xl border-2 font-semibold text-base transition-all duration-300 active:scale-95 ${
                  isSelected
                    ? 'border-[#667eea] bg-gradient-to-br from-[#667eea]/25 to-[#764ba2]/15 text-white shadow-lg shadow-[#667eea]/20'
                    : 'border-white/15 bg-white/5 text-white/80 hover:bg-white/10 hover:border-white/25 hover:shadow-md'
                }`}
                style={{ overflow: 'visible' }} // Fix tick cut off
              >
                <span className="flex items-center justify-center h-full px-4">{g.label}</span>
                {isSelected && (
                  <div className="absolute top-[-18px] right-[-18px] pointer-events-none" style={{zIndex:10}}>
                    <div className="bg-gradient-to-r from-emerald-400 to-green-500 rounded-full p-2 border-2 border-white drop-shadow-lg">
      ...
      <div className="fixed left-0 right-0 bottom-0 z-10 flex justify-between items-center px-6 py-4 border-t border-white/5 bg-[#0B1120]" style={{paddingBottom:'env(safe-area-inset-bottom)'}}>
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-base font-medium text-white/50 hover:text-white/80 transition-colors"
        >
        ...
        <button
          ...
          className={`flex items-center gap-3 px-8 py-3 rounded-full font-semibold text-base transition-all duration-300 ${
            canProceed
  ...
  return (
    <div className="h-full flex flex-col px-6 py-3 pb-[calc(70px+env(safe-area-inset-bottom))]">
      ...
      <div className="flex-none relative max-w-2xl mx-auto mb-4 w-full">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-[#667eea]" style={{filter:'none',opacity:1}} />
        <input
          ref={searchInputRef}
          type="text"
          ...
          className="w-full pl-12 pr-12 py-3.5 rounded-xl border-2 border-[#667eea]/35 bg-white/5 text-base text-white placeholder-white/40 focus:outline-none focus:border-[#667eea] focus:ring-4 focus:ring-[#667eea]/15 transition-all backdrop-blur-sm"
        />
        {searching && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2">
            <Loader2 className="h-5 w-5 text-[#667eea] animate-spin" style={{filter:'none'}} />
          </div>
        )}
        {query && !searching && (
          <button
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar -mx-2 px-2" style={{paddingBottom:'80px'}}>
        {/* Empty State */}
        {!query && favoriteMovies.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-8 opacity-60">
            <Search className="h-16 w-16 mb-4 text-[#667eea]" style={{filter:'none'}} />
            ...
            <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
              {results.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className={`flex w-full items-center gap-4 px-5 py-4 hover:bg-white/5 transition-all border-b border-white/5 last:border-0 text-left group relative`}
                  >
                    <img
                    ...
                    {selected ? (
                      <div className="bg-emerald-500 rounded-full p-2 absolute top-2 right-2">
                      ...
                      <button
                        type="button"
                        onClick={() => addMovie(r)}
                        className="flex items-center justify-center font-bold text-[#667eea] text-sm bg-white/10 border border-[#667eea]/20 rounded-full px-3 py-1 hover:bg-[#667eea]/10 transition-colors"
                      >
                        Add
                      </button>
                    ) : null}
        ...
        {favoriteMovies.length > 0 && (
          <div className="max-w-5xl mx-auto pb-6">
            <div className="flex items-center justify-between mb-3 sticky top-0 z-10 bg-[#0B1120]/95 backdrop-blur-sm py-2 border-b border-white/5">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3">
              {favoriteMovies.map((m, idx) => (
                <div
                  key={m.id}
                  className="group relative aspect-[2/3] rounded-xl overflow-hidden shadow-lg ring-1 ring-white/10 hover:ring-[#667eea]/50 transition-all duration-300 hover:scale-105 bg-[#101624]"
                  style={{
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 h-8 w-8 rounded-full bg-red-600 hover:bg-red-700 flex items-center justify-center shadow-lg transition-transform hover:scale-110 z-10"
                  >
                    <X className="h-4 w-4 text-white stroke-[3]" />
                  </button>
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-2 text-white font-semibold text-xs rounded-xl">
                    {m.title}
                  </div>
                </div>
              ))}
      ...
      <div className="fixed left-0 right-0 bottom-0 z-10 flex justify-between items-center px-6 py-4 border-t border-white/5 bg-[#0B1120]" style={{paddingBottom:'env(safe-area-inset-bottom)'}}>
        <button
          onClick={onBack}
          disabled={loading}
          className="flex items-center gap-2 text-base font-medium text-white/50 hover:text-white/80 transition-colors disabled:opacity-40"
        >
          <ChevronLeft className="h-4 w-4" />
          Back
        </button>
        <button
          onClick={onFinish}
          disabled={loading || !canProceed}
          className={`flex items-center gap-3 px-10 py-3.5 rounded-full font-semibold text-base transition-all duration-300 ${
            canProceed
              ? 'bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white shadow-lg hover:shadow-xl hover:scale-105 active:scale-95'
              : 'bg-white/5 text-white/30 cursor-not-allowed'
          }`}

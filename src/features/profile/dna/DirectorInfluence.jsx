...
export default function DirectorInfluence({ directors = [], subjectName = null }) {
  ...
  const trust = subjectName ? 'The voices they trust.' : 'The voices you trust.'
  const returnTo = subjectName ? 'the directors they return to most' : 'the directors you return to most'
  const strongest = subjectName ? 'Their strongest repeat pattern' : 'Your strongest repeat pattern'
  return (
      ...
      <div className="ff-dna__shell">
        <div className="ff-dna-section__head">
          <div>
            ...
            <h2 id="ff-dna-voices-h2">{trust}</h2>
          </div>
          <p>Shown through {returnTo}, among filmmakers with at least two films of evidence.</p>
        </div>
        <div className="ff-dna-influence">
          ...
          <aside className="ff-dna-influence__card">
            ...
            <h3>{top.name}</h3>
            <p>{strongest} among eligible directors — {factual(top)}{top.firstWatchedYear ? `, first logged in ${top.firstWatchedYear}` : ''}.</p>

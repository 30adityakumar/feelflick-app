      ...
      <div className="ff-profile-mixtape-grid" style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:18 }}>
        {mixtape.map((f, i) => (
          <button
            ...
            style={{ ...RESET_BTN }}
          >
            <div style={{ position:'relative', borderRadius:6, overflow:'hidden', marginBottom:14, boxShadow:'0 16px 36px -12px rgba(0,0,0,0.6)' }}>
              {f.poster ? (
        ...
        {Array.from({ length: Math.max(0, 5 - mixtape.length) }).map((_, idx) => (
          <div
            key={`empty-${idx}`}
            aria-hidden
            style={{ borderRadius:6, border:`1px dashed ${HP.border}`, aspectRatio:'2/3', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Outfit', fontSize:11, color:HP.textFaint, letterSpacing:'0.12em', textTransform:'uppercase', textAlign:'center', padding:14 }}
          >
            Rate 4.5★+<br/>to fill
          </div>
        ))}

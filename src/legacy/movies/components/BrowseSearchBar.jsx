            ...
            <button
              ...
              className={`inline-flex h-8 items-center rounded-full border px-3.5 text-[0.76rem] font-medium transition-all duration-150 ${
                ...
                active
                  ...
                  : 'border-white/10 bg-white/4 text-white/60 hover:border-white/18 hover:text-white/80'
              }`}
            >
              {opt.label}
            </button>
...
export default function BrowseSearchBar({
  ...
  return (
    <div className="pb-3">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 pt-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="h-11 w-full rounded-full border border-white/10 bg-white/6 pl-11 pr-10 text-[0.92rem] text-white placeholder:text-white/40 transition-colors focus:border-purple-400/40 focus:bg-white/9 focus:outline-none"
          />
          {draftQuery && (
            <button
              type="button"
      ...
      {/* ── Row 2: Quick presets ─────────────────────────────────────────── */}
      <div className="mt-2 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] scrollbar-none [&::-webkit-scrollbar]:hidden">
        {PRESETS.map(preset => (
          <button
            key={preset.id}
            type="button"
            ...
            className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap border transition-all duration-150 ${
              activePreset === preset.id
                ...
                : 'bg-white/4 border-white/8 text-white/60 hover:text-white/80 hover:border-white/20'
            }`}
          >
            {preset.label}
          </button>
      ...
      <div className="mt-2.5 flex items-center gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] scrollbar-none [&::-webkit-scrollbar]:hidden">
        <SlidersHorizontal
        ...
        {user && (
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
              hideWatched
                ? 'bg-purple-500/20 border-purple-500/40 text-purple-300'
                : 'bg-white/4 border-white/10 text-white/60 hover:text-white/80'
            }`}
          >
        ...
        {hasAnyFilter && (
          <button
            ...
            className="ml-1 inline-flex h-9 shrink-0 items-center gap-1 rounded-full border border-white/10 bg-white/4 px-3 text-[0.76rem] font-medium text-white/40 transition-colors hover:text-white/70"
          >
            <X className="h-3 w-3" />
            Clear all
          </button>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className={isSearchMode ? 'opacity-40' : ''}>
              ...
              <input
                ...
                disabled={isSearchMode}
                className="h-9 w-full rounded-lg border border-white/10 bg-white/6 px-3 text-sm text-white placeholder:text-white/20 focus:border-purple-400/40 focus:outline-none disabled:cursor-not-allowed"
              />
            </div>

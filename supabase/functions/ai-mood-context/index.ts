...
// limiter). Anon-reachable (public /discover), so it rate-limits by a hashed IP, not a user.
const FN_NAME = 'ai-mood-context'
const COOLDOWN_SECS = Number(Deno.env.get('MOOD_COOLDOWN_SECS') ?? '3')
const DAILY_CAP     = Number(Deno.env.get('MOOD_DAILY_CAP') ?? '100')
...
Deno.serve(async (req: Request) => {
  ...
  // Durable per-caller rate limit + global budget (anon path), replacing the in-memory IP limiter.
  ...
  if (gateClient) {
    ...
    if (!gate.allowed) return new Response('Too Many Requests', { status: 429, headers })
  }
